...
{
	btAssert(NULL == sAlignedAllocFunc);
	sAlignedAllocFunc = allocFunc;
    btAssert(NULL == sAlignedFreeFunc);
...
void *btAlignedAllocInternal(size_t size, int raw_alignment, int line, const char *filename)
{
	btAssert(0 == (raw_alignment & (raw_alignment - 1)));
	int alignment = (raw_alignment > 16) ? raw_alignment : 16;
	...
	char *real = (char *)sAlignedAllocFunc(size + sz4prt + (alignment - 1), 16);
...
void btAlignedFreeInternal(void *ptr, int line, const char *filename)
{
	...
	sAlignedFreeFunc(real);
}

#else  //BT_DEBUG_MEMORY_ALLOCATIONS

void *btAlignedAllocInternal(size_t size, int raw_alignment)
{
	btAssert(0 == (raw_alignment & (raw_alignment - 1)));
	int alignment = (raw_alignment > 16) ? raw_alignment : 16;
